...
mkdir -p data/bam/${NAME}
mkdir -p data/sam/${NAME}
mkdir -p results/logs/$SLURM_JOB_NAME-$SLURM_JOB_ID

echo "Moving trimmed FASTQ data to the working directory"
cp -r /home/gao.chenx/BINF_data_pipeline/shortReadAlignment/data/trimmed_data/${NAME}/${SRR_ID}.fastq data/trimmed/${NAME}
echo "Moving alignment files to working directory"
cp -r /home/gao.chenx/BINF_data_pipeline/shortReadAlignment/data/aligned_data/${NAME}/${NAME}.sorted.bam data/bam/${NAME}
cp -r /home/gao.chenx/BINF_data_pipeline/shortReadAlignment/data/aligned_data/${NAME}/${NAME}.sam data/sam/${NAME}
...
bash scripts/runTrinity.sh $NAME 1>results/logs/$SLURM_JOB_NAME-$SLURM_JOB_ID/$SLURM_JOB_NAME-$SLURM_JOB_ID-runTrinity.log 2>results/logs/$SLURM_JOB_NAME-$SLURM_JOB_ID/$SLURM_JOB_NAME-$SLURM_JOB_ID-runTrinity.err

echo "Analyze the Guided Transcriptome $(date)"
bash scripts/analyzeTrinity.sh $NAME 1>results/$NAME/$SLURM_JOB_NAME-$SLURM_JOB_ID-trinity_guided_stats.txt 2>results/logs/$SLURM_JOB_NAME-$SLURM_JOB_ID/$SLURM_JOB_NAME-$SLURM_JOB_ID-analyzeTrinity.err
...
bash scripts/trinityDeNovoSE.sh $NAME $SRR_ID 1>results/logs/$SLURM_JOB_NAME-$SLURM_JOB_ID/$SLURM_JOB_NAME-$SLURM_JOB_ID-trinityDeNovoSE.log 2>results/logs/$SLURM_JOB_NAME-$SLURM_JOB_ID/$SLURM_JOB_NAME-$SLURM_JOB_ID-trinityDeNovoSE.err
